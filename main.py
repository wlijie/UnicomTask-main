...
def readJson():
    ...
    # 710e19cae85c48fc9fb0f099d661b690
    if "ENCKEY" in os.environ:
        key = os.environ['ENCKEY']
        aes = AEScoder(key);
        encfile = open("/".join([sys.path[0], "config_of_mine.json.enc"])).read();
        plaintxt = aes.decrypt(encfile);
        users = json.loads(plaintxt)
        return users
